...
#define TOTP_KEY_SIZE SHA256_DIGEST_SIZE // Size of TOTP key in bytes
...
#define SPDM_TOTP_USE_PSK 0

// SEND_SPDM_HEARTBEAT = 1 => send heartbeats on loop
// SEND_SPDM_HEARTBEAT = 0 => run TOTP on loop
#define SEND_SPDM_HEARTBEAT 0
...
static void totp_spdm_work_handler(struct work_struct *w) {
	...
	pr_info("counter PERF_COUNT_SW_TASK_CLOCK:%d PERF_COUNT_HW_INSTRUCTIONS:%d PERF_COUNT_HW_CPU_CYCLES:%d ",
				MY_PERF_TYPE == PERF_TYPE_SOFTWARE && (uint32_t) MY_PERF_CONFIG == (uint32_t) PERF_COUNT_SW_TASK_CLOCK,
				MY_PERF_TYPE == PERF_TYPE_HARDWARE && (uint32_t) MY_PERF_CONFIG == (uint32_t) PERF_COUNT_HW_INSTRUCTIONS,
				MY_PERF_TYPE == PERF_TYPE_HARDWARE && (uint32_t) MY_PERF_CONFIG == (uint32_t) PERF_COUNT_HW_CPU_CYCLES);
	...
	pe.type = MY_PERF_TYPE;		//PERF_TYPE_SOFTWARE;
	pe.config = MY_PERF_CONFIG;	//PERF_COUNT_SW_TASK_CLOCK;
	...
	totp_spdm_usb_struct->spdm_status = do_authentication_via_spdm_totp(totp_spdm_usb_struct->spdm_context);
	if (RETURN_ERROR(totp_spdm_usb_struct->spdm_status)) {
	...
	printk(KERN_INFO "SPDM session started (%s). counter: %llu\n", (use_psk?"PSK":"asym"), counter_spdm_create_session);
	...
	while(SEND_SPDM_HEARTBEAT==1){
		...
		perf_event_release_kernel(event_totp_challenge);
		printk(KERN_INFO "SPDM hearbeat done. counter: %llu\n", counter_totp_challenge);
	}

	while(SEND_SPDM_HEARTBEAT==0){
